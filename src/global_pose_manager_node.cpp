...
private:
    ...
    // 1. 引入 TF 监听器，实现历史时间戳的回溯 (解决 Time Machine Bug)
    ...
    // 外部控制阀门
    bool is_algorithm_enabled_; // 逻辑总闸 (同事控制)
    ...
    void resetSystemState() {
        ...
        ROS_WARN("[GlobalPoseManager] System state has been reset to INIT.");
    }
...
public:
    GlobalPoseManager() : nh_("~"), current_state_(STATE_0_INIT), tf_listener_(tf_buffer_),
        ...
        ROS_INFO("Global Pose Manager (Industrial Grade) Initialized.");
    }
    ...
    void eStopCallback(const std_msgs::Int8::ConstPtr& msg) {
        ...
        if (current_estop_state && !is_estop_active_) {
            ROS_ERROR("[GlobalPoseManager] emergency stop on!");
            is_estop_active_ = true;
            resetSystemState(); 
        } else if (!current_estop_state && is_estop_active_) {
            ROS_INFO("\033[1;32m[GlobalPoseManager] emergency stop off\033[0m");
            is_estop_active_ = false;
        }
    }
    ...
    bool toggleAlgorithmCallback(std_srvs::SetBool::Request& req, std_srvs::SetBool::Response& res) {
        ...
        if (is_algorithm_enabled_) {
            ROS_INFO("\033[1;32m[GlobalPoseManager] map_cleaning completed.\033[0m");
            std::cout << "[GlobalPoseManager] map_cleaning completed." << std::endl;
            res.message = "Manager Enabled.";
        } else {
            ROS_WARN("[GlobalPoseManager] map cleaning");
            std::cout << "[GlobalPoseManager] map cleaning" << std::endl;
            resetSystemState(); 
    ...
    void PoseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg) {
        ...
        try {
            ...
            ROS_WARN_THROTTLE(1.0, "TF Sync Warning: Waiting for historical odom... %s", ex.what());
        ...
        if(current_state_ != STATE_0_INIT && is_confident) {
            ...
            if(jump_dist > 0.5 || jump_angle_rad > 0.52) {
                ROS_ERROR_THROTTLE(1.0, "Spatial gate triggered! Anomalous jump detected. Forcibly discading high-confidence pose!");
        ...
        if(is_fatal) {
           // 【断头台触发】：无视任何计数器，立刻强行切入盲走自保！
            if (current_state_ != STATE_2_BLIND_ODOM) {
                ...
                ROS_FATAL("[GlobalPoseManager] FATAL ERROR: AMCL crashed, switch to blind odom mode.");
            }
        ...
        // ----- 状态切换 -----
        if (current_state_ == STATE_0_INIT && confident_frames_count_ >= 1) {
        ...
        else if (current_state_ == STATE_1_TRACKING || current_state_ == STATE_3_RECOVERY) {
            if (lost_frames_count_ >= 3) { // 3帧恶化就判定丢锁
                lost_frames_count_ = 0;
                confident_frames_count_ = 0;
                current_state_ = STATE_2_BLIND_ODOM;
                executeAutoRescue(msg->header.stamp);
                ROS_WARN("State: TRACKING -> BLIND_ODOM. Freezing TF target.");
            } else if(!is_degraded) {
    ...
    bool rescueCallback(std_srvs::Empty::Request& req, std_srvs::Empty::Response& res) {
        ...
        ROS_INFO("\033[1;32m[GlobalPoseManager] Successfully published a recovery pose! AMCL has been forcibly brought back on track. \033[0m");
        return true;
    }
    ...
    void odomCallback(const nav_msgs::Odometry::ConstPtr& msg) {
        ...
        if (current_state_ == STATE_1_TRACKING || current_state_ == STATE_3_RECOVERY) {
    ...
    void executeAutoRescue(const ros::Time& current_stamp) {
        ...
        // 如果距离上次救援还不到 5.0 秒，绝对不允许再次发射，给 AMCL 留出收敛的时间！
        if (!last_rescue_time_.isZero() && (ros::Time::now() - last_rescue_time_).toSec() < 1.0) {
            ROS_WARN_THROTTLE(1.0, "[GlobalPoseManager] auto-saving is freezing...");
            return;
        }
        ...
        try {
            ...
            ROS_ERROR("[GlobalPoseManager] 自动救援获取 Odom 失败! 原因: %s", ex.what());
            return;
        }
        ...
        ROS_INFO("\033[1;32m[GlobalPoseManager] Auto-Rescue payload delivered! AMCL has been forced to align with Odom tracking.\033[0m");
    }
};
